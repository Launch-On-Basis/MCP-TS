...
async function initClient() {
    ...
    client = await basis_sdk_js_1.BasisClient.create({ privateKey: PRIVATE_KEY });
    if (process.env.BASIS_API_KEY) {
        client._apiKey = process.env.BASIS_API_KEY;
    }
...
const TOOLS = [
    // ── Module 1: Trading (8) ──────────────────────────────
    { name: "buy_token", description: "Buy a token using USDB. Elastic supply — buying increases price. Previews before executing.", inputSchema: { type: "object", properties: { token: { type: "string", description: "Token name (STASIS) or address" }, amount_usdb: { type: "number", description: "USDB to spend" }, slippage_percent: { type: "number", description: "Max slippage % (default: 1)" }, wrap: { type: "boolean", description: "Wrap output to wSTASIS (default: false)" } }, required: ["token", "amount_usdb"] } },
    { name: "sell_token", description: "Sell a token for USDB. Checks balance first. Use percentage to scale out.", inputSchema: { type: "object", properties: { token: { type: "string", description: "Token name or address" }, amount: { type: "number", description: "Token amount to sell" }, percentage: { type: "number", description: "1-100, sell this % of balance" }, slippage_percent: { type: "number", description: "Max slippage % (default: 1)" }, to_usdb: { type: "boolean", description: "Swap all the way to USDB (default: true). False stops at MAINTOKEN." } }, required: ["token"] } },
    ...
    // ── Sync helpers ───────────────────────────────────
    { name: "sync_faucet", description: "Sync a faucet claim transaction.", inputSchema: { type: "object", properties: { tx_hash: { type: "string" } }, required: ["tx_hash"] } },
    ...
    // ── Utility ────────────────────────────────────────
    { name: "claim_faucet", description: "Claim test USDB from faucet (one per wallet). Optionally set a referrer.", inputSchema: { type: "object", properties: { referrer: { type: "string", description: "Referrer wallet address (optional)" } } } },
    { name: "set_referrer", description: "Set a referrer for your wallet. One-time only — reverts if already set.", inputSchema: { type: "object", properties: { referrer: { type: "string", description: "Referrer wallet address" } }, required: ["referrer"] } },
...
async function handleTool(name, args) {
    try {
        switch (name) {
            ...
            // ── Sync helpers ───────────────────────────────────
            case "sync_faucet": {
                return ok(await client.api.syncFaucet(args.tx_hash));
            }
            case "sync_loan": {
            ...
            // ── Utility ────────────────────────────────────────
            case "claim_faucet": {
                const tx = await client.claimFaucet(args.referrer || '0x0000000000000000000000000000000000000000');
                return txResult(tx, { message: "Claimed test USDB" });
            }
            case "sync_transaction": {
                return ok(await client.api.syncTransaction(args.tx_hash));
            }
            case "set_referrer": {
                const tx = await client.setReferrer(args.referrer);
                return txResult(tx);
            }
