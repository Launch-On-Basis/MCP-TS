...
const TOOLS = [
    ...
    { name: "pm_get_min_seed_public", description: "Get min seed amount (USDB) for a non-private private market. Higher than private-only.", inputSchema: { type: "object", properties: {} } },
    { name: "pm_get_min_seed_private", description: "Get min seed amount (USDB) for a voter-panel private market. Often 0.", inputSchema: { type: "object", properties: {} } },
    // ── Extra methods ──────────────────────────────────
    ...
    { name: "get_min_seed", description: "Get minimum seed amount (USDB) required to create a public prediction market. createMarket reverts below this.", inputSchema: { type: "object", properties: {} } },
    ...
    // ── Utility ────────────────────────────────────────
    { name: "claim_faucet", description: "Claim daily USDB from faucet (up to 500 USDB/day based on eligibility signals). Requires SIWE session. Check get_faucet_status first.", inputSchema: { type: "object", properties: { referrer: { type: "string", description: "Referrer wallet address (optional)" } } } },
    ...
    try {
        switch (name) {
            ...
            case "create_token": {
                ...
                const imageFile = args.image_file_path ? (0, fs_1.readFileSync)(args.image_file_path) : undefined;
            ...
            case "get_min_seed": {
                const seed = await client.predictionMarkets.getMinSeed();
                return ok({ min_seed_usdb: fromRaw(seed) });
            }
            case "pm_get_min_seed_public": {
                const seed = await client.privateMarkets.getMinSeedPublic();
                return ok({ min_seed_usdb: fromRaw(seed) });
            }
            case "pm_get_min_seed_private": {
                const seed = await client.privateMarkets.getMinSeedPrivate();
                return ok({ min_seed_usdb: fromRaw(seed) });
            }
