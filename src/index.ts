...
const TOOLS = [
  ...
  // ── Module 1: Trading (8) ──────────────────────────────
  { name: "buy_token", description: "Buy a token using USDB. Elastic supply — buying increases price. Previews before executing.", inputSchema: { type: "object" as const, properties: { token: { type: "string", description: "Token name (STASIS) or address" }, amount_usdb: { type: "number", description: "USDB to spend" }, slippage_percent: { type: "number", description: "Max slippage % (default: 1)" }, wrap: { type: "boolean", description: "Wrap output to wSTASIS (default: false)" } }, required: ["token", "amount_usdb"] } },
  { name: "sell_token", description: "Sell a token for USDB. Checks balance first. Use percentage to scale out.", inputSchema: { type: "object" as const, properties: { token: { type: "string", description: "Token name or address" }, amount: { type: "number", description: "Token amount to sell" }, percentage: { type: "number", description: "1-100, sell this % of balance" }, slippage_percent: { type: "number", description: "Max slippage % (default: 1)" }, to_usdb: { type: "boolean", description: "Swap all the way to USDB (default: true). False stops at MAINTOKEN." } }, required: ["token"] } },
  ...
  { name: "get_token_list", description: "List/search tokens.", inputSchema: { type: "object" as const, properties: { search: { type: "string" }, limit: { type: "number" } } } },
  ...
  { name: "register_agent", description: "Register as AI agent on-chain (ERC-8004).", inputSchema: { type: "object" as const, properties: { name: { type: "string", description: "Agent display name" }, description: { type: "string", description: "Agent description" }, capabilities: { type: "array", items: { type: "string" } } }, required: ["name"] } },
  ...
  { name: "verify_twitter", description: "Verify a Twitter challenge tweet.", inputSchema: { type: "object" as const, properties: { tweet_url: { type: "string" } }, required: ["tweet_url"] } },
...
async function handleTool(name: string, args: any): Promise<any> {
  try {
    switch (name) {
      ...
      case "get_token_list": { return ok((await client.api.getTokens({ search: args.search, limit: args.limit || 20 })).data); }
      ...
      case "upload_image_from_url": { return ok({ url: await client.api.uploadImageFromUrl(args.image_url, args.contract_address) }); }
