...
const TOOLS = [
  ...
  { name: "get_vault_loan", description: "Get full loan details for an active vault loan. Handles the vault-as-borrower indirection automatically. Returns null if no active vault loan.", inputSchema: { type: "object" as const, properties: { wallet: { type: "string", description: "User wallet address (default: your wallet)" } } } },
  ...
  { name: "get_my_orders", description: "Get your order book history. Paginated with optional status/market/outcome filters.", inputSchema: { type: "object" as const, properties: { status: { type: "string", description: "Filter by status (e.g. open, filled, canceled)" }, market_token: { type: "string", description: "Filter to a specific market" }, outcome_id: { type: "number", description: "Narrow to a single outcome (0-indexed)" }, page: { type: "number" }, limit: { type: "number" } } } },
  ...
  { name: "get_creator_earnings", description: "Get un-distributed accrued USDB earnings for a dev on a specific token. Reads the pending balance before next distribution.", inputSchema: { type: "object" as const, properties: { token: { type: "string", description: "Token contract address" }, dev: { type: "string", description: "Dev wallet address (default: your wallet)" } }, required: ["token"] } },
  { name: "get_dev_total_earnings", description: "Get a dev's lifetime distributed USDB earnings across every token they have a share on.", inputSchema: { type: "object" as const, properties: { dev: { type: "string", description: "Dev wallet address (default: your wallet)" } } } },
  ...
  { name: "updown_get_round", description: "Get a specific UPDOWN round by id — or omit round_id to get the current round. Returns the full Round struct (startPrice/endPrice in 8-dec, pools/shares/seedBonus in 18-dec USDB, outcome enum 0=Pending,1=BullWins,2=BearWins,3=Canceled). Returns null if no rounds opened yet.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number", description: "Timeframe enum: 0=5m,1=15m,2=1h,3=4h,4=24h" }, round_id: { type: "number", description: "Optional — defaults to currentRoundId(tf)" } }, required: ["asset","tf"] } },
  { name: "updown_get_user_bet", description: "Get your bet on a specific UPDOWN round. amount=0 means no bet placed.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, round_id: { type: "number" } }, required: ["asset","tf","round_id"] } },
  { name: "updown_quote_shares", description: "Preview shares minted by a hypothetical bet on the current round. Includes slippage. Returns 0 if no active round.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, side: { type: "string", enum: ["bull","bear"] }, amount: { type: "number", description: "USDB amount (whole tokens)" } }, required: ["asset","tf","side","amount"] } },
  { name: "updown_quote_claim", description: "Exact USDB you can claim from a settled round. 0 means nothing to claim (lost / already claimed / pending / no bet).", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, round_id: { type: "number" } }, required: ["asset","tf","round_id"] } },
  { name: "updown_bet", description: "Place a bullish or bearish bet on the current UPDOWN round. Auto-approves USDB and pre-checks balance + minBet client-side. Use slippage_percent for an optional minShares guard.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, side: { type: "string", enum: ["bull","bear"] }, amount: { type: "number", description: "USDB amount (whole tokens)" }, slippage_percent: { type: "number", description: "Optional. If set, throws when projected shares would be > slippage_percent below quoteShares (default: no slippage check)" } }, required: ["asset","tf","side","amount"] } },
  { name: "updown_claim", description: "Claim winnings or refund from a settled UPDOWN round. Pre-checks via quoteClaimPayout — refuses to send if 0.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, round_id: { type: "number" } }, required: ["asset","tf","round_id"] } },
  { name: "updown_settle", description: "Public settle of an UPDOWN round once it has ended and a valid Chainlink price is available. Anyone can call.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" } }, required: ["asset","tf"] } },
  { name: "updown_cancel_stalled", description: "Public cancel of a stalled UPDOWN round (settle window expired). Anyone can call after endTime + FINALIZE_WINDOW.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" } }, required: ["asset","tf"] } },
  { name: "updown_my_history", description: "Aggregate UPDOWN bet/claim summary for your wallet across every asset (BTC/ETH/BNB/CAKE/DOGE) and all timeframes — totals, win/loss tallies, and embedded activeBets[] / claimableBets[] arrays.", inputSchema: { type: "object" as const, properties: {} } },
  { name: "updown_list_rounds", description: "Paginated UPDOWN round list for a token, newest-first. Optional tf and outcome filters.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" }, outcome: { type: "string", enum: ["pending","bull_wins","bear_wins","canceled"] }, cursor: { type: "string", description: "nextCursor from a prior page" }, limit: { type: "number", description: "default 20, max 200" } }, required: ["asset"] } },
  { name: "updown_bull_probability", description: "Current bull probability for the active UPDOWN round (basis points 0-10000, also returned as percentage).", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number", description: "0=5m, 1=15m, 2=1h, 3=4h, 4=24h" } }, required: ["asset","tf"] } },
  { name: "updown_quote_current_payout", description: "Estimate your payout if the active UPDOWN round settled now in your favor. Returns 0 if no bet placed.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" } }, required: ["asset","tf"] } },
  { name: "updown_min_bet", description: "Minimum bet amount in USDB for an UPDOWN asset.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] } }, required: ["asset"] } },
  { name: "updown_tf_duration", description: "Round duration in seconds for a timeframe.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" } }, required: ["asset","tf"] } },
  { name: "updown_paused", description: "Check if an UPDOWN asset is paused (writes will revert if true).", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] } }, required: ["asset"] } },
  { name: "updown_slippage_threshold", description: "Current slippage threshold in BPS for the active round. Decays from 9500 (95%) at start to 5500 (55%) at end.", inputSchema: { type: "object" as const, properties: { asset: { type: "string", enum: ["btc","eth","bnb","cake","doge"] }, tf: { type: "number" } }, required: ["asset","tf"] } },
];
...
const UPDOWN_ASSETS = ["btc", "eth", "bnb", "cake", "doge"] as const;
function resolveUpDownAsset(asset: any): any {
  if (!UPDOWN_ASSETS.includes(asset)) throw new Error(`asset must be one of ${UPDOWN_ASSETS.join(", ")} (got ${asset})`);
  const m = (client as any).updown?.[asset];
  if (!m) throw new Error(`UPDOWN ${asset} not deployed yet.`);
  return m;
}
function validateTf(tf: any): boolean {
  return Number.isInteger(tf) && tf >= 0 && tf <= 4;
}
function sideToInt(side: any): number {
  if (side === "bull") return 1;
  if (side === "bear") return 2;
  return 0;
}
function validateRoundId(roundId: any): boolean {
  ...
  try {
    switch (name) {
      ...
      case "get_vault_loan": {
        ...
        const loan = await (client.staking as any).getVaultLoan(w);
      ...
      case "take_loan": {
        ...
        const amount = toRaw(args.amount);
        const balance = await client.publicClient.readContract({ address: tokenAddr, abi: erc20Abi, functionName: "balanceOf", args: [walletAddress] }) as bigint;
        if (amount > balance) return err(`Insufficient collateral balance. Have: ${fromRaw(balance)}, want: ${args.amount}. Tip: prediction-market tokens take a 1.5% tax on buy — use the post-tax balance, not the buy preview.`);
      ...
      case "claim_leverage_liquidation": {
        const tx = await (client.trading as any).claimLeverageLiquidation(BigInt(args.loan_id));
      ...
      case "get_my_orders": {
        ...
        if (args.market_token) opts.marketToken = args.market_token;
        if (args.outcome_id !== undefined) opts.outcomeId = args.outcome_id;
        if (args.page !== undefined) opts.page = args.page;
        if (args.limit !== undefined) opts.limit = args.limit;
        return ok(await (client.api as any).getMyOrders(opts));
      }
      ...
      case "get_creator_earnings": {
        const dev = (args.dev || walletAddress) as Address;
        const earnings = await (client.taxes as any).getCreatorEarnings(args.token as Address, dev) as bigint;
        return ok({ token: args.token, dev, accrued_usdb: fromRaw(earnings) });
      }
      case "get_dev_total_earnings": {
        const dev = (args.dev || walletAddress) as Address;
        const total = await (client.taxes as any).getDevTotalEarnings(dev) as bigint;
      ...
      case "updown_get_round": {
        ...
        if (!(m as any).address) return m as any;
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        let roundId: bigint;
        if (args.round_id === undefined || args.round_id === null) {
          roundId = await (m as any).currentRoundId(args.tf) as bigint;
        } else {
          if (!validateRoundId(args.round_id)) return err(`round_id must be a positive integer (got ${JSON.stringify(args.round_id)})`);
          roundId = BigInt(args.round_id);
        }
        if (roundId === 0n) return ok(null);
        const round = await (m as any).getRound(args.tf, roundId);
      ...
      case "updown_get_user_bet": {
        ...
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        if (!validateRoundId(args.round_id)) return err(`round_id must be a positive integer (got ${JSON.stringify(args.round_id)})`);
        const bet = await (m as any).getUserBet(args.tf, BigInt(args.round_id), walletAddress);
        return ok(bet);
      }
      case "updown_quote_shares": {
        ...
        if (!(m as any).address) return m as any;
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const sideInt = sideToInt(args.side); if (!sideInt) return err(`side must be "bull" or "bear" (got ${JSON.stringify(args.side)})`);
        const shares = await (m as any).quoteShares(args.tf, sideInt, toRaw(args.amount));
        return ok({ shares: fromRaw(shares as bigint), sharesRaw: (shares as bigint).toString() });
      }
      case "updown_quote_claim": {
        ...
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        if (!validateRoundId(args.round_id)) return err(`round_id must be a positive integer (got ${JSON.stringify(args.round_id)})`);
        const payout = await (m as any).quoteClaimPayout(args.tf, BigInt(args.round_id), walletAddress);
        return ok({ claimable_usdb: fromRaw(payout as bigint), claimable_raw: (payout as bigint).toString() });
      }
      case "updown_bet": {
        const m = resolveUpDownAsset(args.asset);
        if (!(m as any).address) return m as any;
        ...
        if (args.slippage_percent !== undefined && args.slippage_percent !== null) {
          ...
          if (!Number.isFinite(slip)) return err(`slippage_percent must be a number (got ${JSON.stringify(args.slippage_percent)})`);
          if (slip < 0 || slip > 100) return err(`slippage_percent must be in [0, 100] (got ${slip})`);
          if (slip > 0) {
            const projected = await (m as any).quoteShares(args.tf, sideInt, amount) as bigint;
            const keepBps = BigInt(Math.round((100 - slip) * 100)); // e.g. 1.5% -> 9850
            minShares = projected * keepBps / 10000n;
          }
        }
        const tx = sideInt === 1
          ? await (m as any).betBull(args.tf, amount, minShares)
          : await (m as any).betBear(args.tf, amount, minShares);
      ...
      case "updown_claim": {
        ...
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        if (!validateRoundId(args.round_id)) return err(`round_id must be a positive integer (got ${JSON.stringify(args.round_id)})`);
        const tx = await (m as any).claim(args.tf, BigInt(args.round_id));
      ...
      case "updown_settle": {
        ...
        if (!(m as any).address) return m as any;
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const tx = await (m as any).settleCurrentRound(args.tf);
      ...
      case "updown_cancel_stalled": {
        ...
        if (!(m as any).address) return m as any;
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const tx = await (m as any).cancelCurrentRoundAndStartNext(args.tf);
        return txResult(tx, { asset: args.asset, tf: args.tf });
      }
      case "updown_my_history": {
        return ok(await (client.api as any).getMyUpDown());
      }
      case "updown_list_rounds": {
        if (!UPDOWN_ASSETS.includes(args.asset)) return err(`asset must be one of ${UPDOWN_ASSETS.join(", ")} (got ${args.asset})`);
        if (args.tf !== undefined && !validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const opts: any = { token: args.asset };
        if (args.tf !== undefined) opts.tf = args.tf;
        if (args.outcome) opts.outcome = args.outcome;
        if (args.cursor) opts.cursor = args.cursor;
        if (args.limit !== undefined) opts.limit = args.limit;
        return ok(await (client.api as any).getUpDownRounds(opts));
      }

      case "updown_bull_probability": {
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const m = resolveUpDownAsset(args.asset);
        const prob = await m.currentBullProbability(args.tf) as bigint;
      ...
      case "updown_quote_current_payout": {
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const m = resolveUpDownAsset(args.asset);
        const payout = await m.quoteCurrentPayout(args.tf, walletAddress) as bigint;
        return ok({ asset: args.asset, tf: args.tf, current_payout_usdb: fromRaw(payout) });
      }

      case "updown_min_bet": {
        const m = resolveUpDownAsset(args.asset);
        const min = await m.minBet() as bigint;
      ...
      case "updown_tf_duration": {
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const m = resolveUpDownAsset(args.asset);
        const dur = await m.tfDuration(args.tf) as bigint;
        return ok({ asset: args.asset, tf: args.tf, duration_seconds: Number(dur) });
      }

      case "updown_paused": {
        const m = resolveUpDownAsset(args.asset);
        const paused = await m.paused() as boolean;
      ...
      case "updown_slippage_threshold": {
        if (!validateTf(args.tf)) return err(`tf must be 0-4 (got ${args.tf})`);
        const m = resolveUpDownAsset(args.asset);
        const bps = await m.currentSlippageThreshold(args.tf) as bigint;
